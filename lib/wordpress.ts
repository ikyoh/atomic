...
export async function getAllAchievements(filterParams?: {
  author?: string;
  secteur?: string;
  tag?: string;
  category?: string;
  ...
  if (filterParams?.search) {
    ...
    if (filterParams?.secteur) {
      query.secteur = filterParams.secteur;
    }
    if (filterParams?.author) {
      query.author = filterParams.author;
    }
  ...
  const url = getUrl("/wp-json/wp/v2/realisations", query);
  return wordpressFetch<Post[]>(url, {
    next: {
      ...defaultFetchOptions.next,
      tags: ["wordpress", "realisations"],
    },
  });
}
...
export async function getTaxonomy(taxonomy: string): Promise<Category> {
  const url = getUrl(`/wp-json/wp/v2/${taxonomy}`);
...
export async function getProductsOfCategoryById(id: number): Promise<Post> {
  const url = getUrl(`/wp-json/wp/v2/produits?categorie=${id}`);
  const response = await wordpressFetch<Post>(url, {
    next: {
      ...defaultFetchOptions.next,
      tags: ["wordpress", `productsofcategory-${id}`],
    },
  });
...
export async function getProductById(id: number): Promise<Post> {
  const url = getUrl(`/wp-json/wp/v2/produits/${id}`);
  const response = await wordpressFetch<Post>(url, {
    next: {
      ...defaultFetchOptions.next,
      tags: ["wordpress", `product-${id}`],
    },
  });
...
export async function getProductCategoryBySlug(
  ...
  const url = getUrl(`/wp-json/wp/v2/categorie?slug=${slug}`);
  const response = await wordpressFetch<Category[]>(url, {
    next: {
      ...defaultFetchOptions.next,
      tags: ["wordpress", `product-category-${slug}`],
    },
  });
...
export async function getCategoriesOfCategoryId(
  ...
  const response = await wordpressFetch<Category[]>(url, {
    next: {
      ...defaultFetchOptions.next,
      tags: ["wordpress", `SubCategories-${id}`],
    },
  });
...
export async function getAllMenus(): Promise<Page[]> {
  const url = getUrl("/wp-json/wp/v2/navigation");
  const response = await wordpressFetch<Page[]>(url, {
    next: {
      ...defaultFetchOptions.next,
...
export async function getProductsCategories(): Promise<Category[]> {
  const url = getUrl("/wp-json/wp/v2/categorie", {
    per_page: 100,
  });
  const categories = await wordpressFetch<Category[]>(url);
  const catsWithIcon = categories.map((category) => ({
    ...category,
    icon: category.acf.icon ? category.acf.icon : null,
  }));
...
// Get all options from ACF options page
export async function getSiteOptions(): Promise<Options> {
  const url = getUrl("/wp-json/wp/v2/options");
  const response = await wordpressFetch<Options>(url, {
    next: {
      ...defaultFetchOptions.next,
      tags: ["wordpress", "options"],
    },
  });
